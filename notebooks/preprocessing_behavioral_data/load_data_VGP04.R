...
M1 <- readMat("../../data/Behavioral_Data/VGP04/A2/VGP04_A2_1_8.mat")
M2 <- readMat("../../data/Behavioral_Data/VGP04/A2/VGP04_Block7.mat")
# The message in the word document that is in that data folder says: "Run 7 has to be replaced by block 7 (mat file)"
...
# # M1 contains in fact all the data, so no point in doing this
# B <- M2$block
# block_n <- length(B[1,1,])
# Block2 <- data.frame()
# for (block_index in 1:block_n){
...
#   B_tmp$block_index <- block_index
#   
#   Block2 <- rbind(Block2, B_tmp[c("participant_id", "group", "session", "block_index", tmp_names)])
# }
...
# start_index <- which(Event$type == "start block 7")
# end_index <- which(Event$type == "end block 7")
# Event[start_index:end_index, ]

# Event seems to already have the events from block 7 included
# Event2 <- data.frame()
# E <- M2$event
# event_n <- length(E[1,1,])
# for (event_index in 1:event_n){
...
#   
#   Event2 <- rbind(Event2, E_tmp[c("participant_id", "group", "session", "event_index", tmp_names)])
# }
